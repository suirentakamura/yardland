...
union emu65x64::REGS		emu65x64::d[128];
...
//StopReason				emu65x64::stop_reason;
...
void emu65x64::step()
{
    ...
    for (Byte i = 0; i < 4; i++) {
        ...
        case 0x100: op_moveba(am_immw()); break;
        }
    }
}
...
void emu65x64::dump(const char *mnem, QWord ea)
{
	...
	std::cout << " {";
	std::cout << ' ' << toHex(getDWord(sp.q + 1), 8);
	std::cout << ' ' << toHex(getDWord(sp.q + 2), 8);
	std::cout << ' ' << toHex(getDWord(sp.q + 3), 8);
	std::cout << ' ' << toHex(getDWord(sp.q + 4), 8);
	std::cout << " }";
    // std::cout << std::endl;
    std::cout << " Z=";
	std::cout << " {";
	std::cout << ' ' << toHex(z[0].q, 16);
	std::cout << ' ' << toHex(z[1].q, 16);
	std::cout << " }";
}
