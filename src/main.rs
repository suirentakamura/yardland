...
pub fn main() {
    ...
    let drom: devices::RomDevice = std::fs::read("assets/test.rgb").unwrap().into();
    ...
    let window = sdl_video.window("Yardland", 1280, 720)
        .position_centered()
        .vulkan()
        .build()
        .unwrap();
    ...
    let mut texture = texture_creator.create_texture(Some(PixelFormatEnum::RGB565), TextureAccess::Streaming, 1280, 720).unwrap();
    ...
    'main: loop {
        ...
        texture.with_lock(None, |pixels: &mut [u8], pitch| {
            ...
            dman.copy_into_slice(0xA0000, pitch * 720, pixels);
        }).unwrap();
